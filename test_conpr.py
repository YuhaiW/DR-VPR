"""
Test Dual-Branch VPR Model on ConPR dataset with proper image resizing
"""
...
from conpr_eval_dataset_rot import InferDataset, evaluateResults
from train_fusion import VPRModel


def extract_features_conpr(model, dataloader, device='cuda'):
    """
    Extract features from ConPR dataset using Dual-Branch VPR
    """
    model.eval()
    features_list = []
...
def main():
    ...
    SEQUENCES =['20230623', '20230531', '20230611', '20230627', '20230628', '20230706', '20230717', '20230803', '20230809', '20230818']
    #['20230623', '20230809']
    
    # ['20230623', '20230531', '20230611', '20230627', '20230628', 
    ...
    THETA_DEGREES = 0.0  # Rotation angle for query trajectory
    OFFSET = [0.0, 0.0]  # Translation offset [x, y]
    YAW_THRESHOLD = 80.0  # Maximum yaw difference for positive samples (degrees)
    
    # ==================== Setup ====================
    print("="*70)
    print("Dual-Branch VPR Model on ConPR Dataset Evaluation")
    print("="*70)
    ...
    # ==================== Load Datasets ====================
    print("\n3. Loading ConPR datasets...")
    datasets = []
    dataloaders = []
    
    for seq in SEQUENCES:
        try:
            # Images will be automatically resized to 320x320 in the dataset
            ...
            print(f"   Make sure the path exists: {DATASET_PATH}{seq}/bev/")
            continue  
    
    if len(datasets) < 2:
        print("\n   ✗ Need at least 2 sequences (1 database + 1 query)")
        print(f"   Successfully loaded sequences: {[datasets[i].seq_name for i in range(len(datasets))]}")
    ...
    for i, (seq, dataloader) in enumerate(zip(SEQUENCES[:len(dataloaders)], dataloaders)):
        ...
        features = extract_features_conpr(model, dataloader, DEVICE)
    ...
    # ==================== Evaluation ====================
    print("\n5. Evaluating with ConPR metrics...")
    ...
    # Save results
    results_file = f'dual_branch_conpr_results_yaw{YAW_THRESHOLD}.txt'
    with open(results_file, 'w') as f:
        f.write("Dual-Branch VPR Model on ConPR Dataset - Results\n")
        f.write("="*70 + "\n\n")
        f.write(f"Model: Dual-Branch Architecture\n")
        f.write(f"  Branch 1: ResNet50 + MixVPR (4096 dim)\n")
        f.write(f"  Branch 2: E2ResNet (C8) + GeM (512 dim)\n")
        f.write(f"  Fusion: Concatenation (4608 dim)\n\n")
    ...
    print(f"✓ Trajectory plots saved to: trajectory_plots_conpr/")
    print(f"✓ Diagnosis matrix saved to: diagnosis_matrix_conpr_yaw{YAW_THRESHOLD}.csv")
