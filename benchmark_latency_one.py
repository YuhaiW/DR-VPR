"""
Single-model latency bench, invoked as subprocess for each baseline.
Usage: python benchmark_latency_one.py <model_name>
"""
import sys
import os
import time
...
@torch.no_grad()
def bench(model, input_size, name):
    model.eval().to(DEVICE).float()
    x = torch.randn(1, 3, *input_size, device=DEVICE, dtype=torch.float32)
...
if name == 'drvpr':
    ...
    bench(model, (320, 320), 'DR-VPR (ours)')

elif name == 'boq':
    os.chdir('/tmp')
    sys.path = [p for p in sys.path if '/home/yuhai/project/DR-VPR' not in p]
    model = torch.hub.load("amaralibey/bag-of-queries", "get_trained_boq",
                           backbone_name="resnet50", output_dim=16384, trust_repo=True)
    bench(model, (322, 322), 'BoQ (resnet50)')

elif name == 'salad':
    os.chdir('/tmp')
    sys.path = [p for p in sys.path if '/home/yuhai/project/DR-VPR' not in p]
    model = torch.hub.load("serizba/salad", "dinov2_salad", trust_repo=True)
    bench(model, (322, 322), 'SALAD (DINOv2)')

elif name == 'crica':
    os.chdir('/tmp')
    sys.path = [p for p in sys.path if '/home/yuhai/project/DR-VPR' not in p]
    model = torch.hub.load('Lu-Feng/CricaVPR', 'trained_model', trust_repo=True)
    bench(model, (224, 224), 'CricaVPR')
